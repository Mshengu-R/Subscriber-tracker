...
const signUp = async (req, res, next)=>{
    ...
    try{
        ...
        const { gender, status, name, email, phone, password, role} = req.body;
        ...
        if(existingUserEmail || existingUserPhone){
            const message = 'User with this email or phone number already exists';
  ...
  const newUsers = await User.create([{ gender, status, name, email, password: hashedPassword, phone, role }], { session })

  // signs the token with the user id and the secret key, and sets the expiration time for the token
  ...
  // if everything goes well then we commit the transaction and end the session   
